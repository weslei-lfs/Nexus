...
async function dbSaveMonth(u,y,m,d) { return supaPost("nexus_months",{username:u,year:y,month:m,data:d,updated_at:new Date().toISOString()},true); }
...
function Dashboard({ user, displayName, onLogout }) {
  ...
  const [meta]=useState(()=>{ const m=loadMeta(user); if(m) return m; const f={originYear:now.getFullYear(),originMonth:now.getMonth()}; saveMeta(user,f); return f; });
  const [viewYear,setViewYear]=useState(now.getFullYear());
  const [viewMonth,setViewMonth]=useState(now.getMonth());

  const [global,setGlobalState]=useState(()=>{
    const g=loadGlobal(user);
    return { categories:g.categories||DEFAULT_CATEGORIES, investments:g.investments||[], theme:g.theme||"Dark Red", currency:g.currency||"BRL" };
  });
  ...
  const carryover=getCarryover(user,viewYear,viewMonth);
  ...
  },[monthData,viewYear,viewMonth,user,dbLoading]);
  useEffect(()=>{ 
    if(dbLoading) return;
  ...
  useEffect(()=>{
    if(dbLoading) return;
    (async()=>{ const md=await dbLoadMonth(user,viewYear,viewMonth); if(md) setMonthData(md); else setMonthData({expenses:[],incomes:[]}); })();
  ...
  const monthly=MONTHS_SHORT.map((m,i)=>{
    const d=(i===viewMonth)?monthData:loadMonth(user,viewYear,i);
    ...
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"auto auto auto auto auto",alignContent:"start" }}>
          ...
          <div style={{ ...card,gridColumn:"5",gridRow:"4",borderLeft:`3px solid ${t.yellow}`,display:"flex",flexDirection:"column",gap:6 }}>
