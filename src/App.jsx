...
function NexusLogo({ accent, size = 36 }) {
  return (
    <svg width={size * 1.15} height={size} viewBox="0 0 92 80" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* ── N shape ── */}
      {/* Left vertical bar */}
      <rect x="2" y="12" width="13" height="60" rx="2" fill={accent}/>
      {/* Left diagonal going down-right */}
      <polygon points="2,12 15,12 49,58 49,72 36,72" fill={accent}/>
      {/* Right diagonal going up-right from bottom center */}
      <polygon points="36,58 49,58 49,72 36,72" fill={accent}/>
      {/* Right vertical bar — starts at arrow tip */}
      <rect x="49" y="18" width="13" height="54" rx="2" fill={accent}/>
      {/* Center diagonal up from center bottom to right top */}
      <polygon points="36,72 49,58 62,18 75,18 62,54 62,72" fill={accent}/>
      {/* Arrow head pointing up-right */}
      <polygon points="55,2 78,2 78,22 70,14 62,22 55,14" fill={accent}/>
      {/* ── Line chart overlay ── */}
      <polyline points="8,68 30,42 55,54" stroke="white" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" opacity="0.95"/>
      <circle cx="8"  cy="68" r="4" fill="white" opacity="0.95"/>
      <circle cx="30" cy="42" r="4" fill="white" opacity="0.95"/>
      <circle cx="55" cy="54" r="4" fill="white" opacity="0.95"/>
    </svg>
  );
}
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"auto auto auto auto auto",alignContent:"start" }}>
          ...
          <FinancialCalendar
            ...
            now={now} t={t} MONTH_NAMES={MONTH_NAMES} card={card}
          />
...
// ─── FINANCIAL CALENDAR ──────────────────────────────────────────────────────
function FinancialCalendar({ viewYear, viewMonth, expenses, incomes, now, t, MONTH_NAMES, card }) {
