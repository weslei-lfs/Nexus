...
// ─── NEXUS LOGO SVG (fiel à imagem) ──────────────────────────────────────────
function NexusLogo({ accent, size = 36 }) {
  const h = size;
  const w = size * 1.1;
  return (
    <svg width={w} height={h} viewBox="0 0 110 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* Perna esquerda do N — barra vertical esquerda */}
      <rect x="4" y="15" width="14" height="70" rx="2" fill={accent}/>
      {/* Diagonal descendo (esquerda para centro) */}
      <polygon points="4,15 18,15 54,65 54,80 38,80" fill={accent}/>
      {/* Diagonal subindo (centro para direita) */}
      <polygon points="54,20 68,20 54,65 38,65" fill={accent}/>
      {/* Barra vertical direita — começa mais alto (ponta da seta) */}
      <rect x="54" y="20" width="14" height="65" rx="2" fill={accent}/>
      {/* Seta para cima no topo direito */}
      <polygon points="61,4 74,20 48,20" fill={accent}/>
      {/* Linha do gráfico branca por cima */}
      <polyline points="10,78 36,45 62,60" stroke="white" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" opacity="0.95"/>
      {/* Pontos brancos */}
      <circle cx="10" cy="78" r="4.5" fill="white" opacity="0.95"/>
      <circle cx="36" cy="45" r="4.5" fill="white" opacity="0.95"/>
      <circle cx="62" cy="60" r="4.5" fill="white" opacity="0.95"/>
    </svg>
  );
}
    ...
    <div style={{ minHeight:"100vh",background:"#0d0d0d",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <div className="fade-in" style={{ background:"#161616",border:"1px solid #242424",borderRadius:20,padding:44,width:400,boxShadow:"0 0 100px rgba(229,57,53,0.12)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8,justifyContent:"center" }}>
          <NexusLogo accent="#e53935" size={52}/>
    ...
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ background:t.header,borderBottom:`1px solid ${t.border}`,padding:"0 18px",height:54,display:"flex",alignItems:"center",gap:10,flexShrink:0,position:"sticky",top:0,zIndex:200 }}>
        <div style={{ display:"flex",alignItems:"center",gap:8,marginRight:4 }}>
          <NexusLogo accent={t.accent} size={38}/>
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"auto auto auto auto",alignContent:"start" }}>
          ...
          {(() => {
            ...
            return (
              <div style={{ ...card,gridColumn:"2/4",gridRow:"3" }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2 }}>
                  {cells.map((d,i)=>{
                    ...
                    let bg = "transparent";
                    let border = `1px solid ${t.border}44`;
                    if(hasBoth) { bg = "linear-gradient(135deg,"+t.green+"33 50%,"+t.red+"33 50%)"; border=`1px solid ${t.yellow}66`; }
                    else if(hasInc) { bg = t.green+"22"; border=`1px solid ${t.green}66`; }
                    else if(hasExp) { bg = t.red+"22"; border=`1px solid ${t.red}66`; }
                    if(isToday) border=`2px solid ${t.accent}`;

                    const tooltip = ev ? [...(ev.incList.map(n=>"✅ "+n)), ...(ev.expList.map(n=>"❌ "+n))].join("
") : "";

                    return (
                      <div key={d} title={tooltip} style={{
                        background:bg, border, borderRadius:5, padding:"3px 2px",
                        textAlign:"center", cursor:ev?"pointer":"default", position:"relative"
                      }}>
                        ...
                        {/* value on hover via title — show small amount */}
                        {ev&&(hasInc||hasExp)&&(
                          <div style={{ fontSize:7,color:hasBoth?t.yellow:hasInc?t.green:t.red,lineHeight:1.1,marginTop:1,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis" }}>
                            {hasBoth?"±":hasInc?"+":"-"}{Math.abs((ev.inc||0)-(ev.exp||0)).toLocaleString("pt-BR",{minimumFractionDigits:0,maximumFractionDigits:0})}
                          </div>
                        )}
                      </div>
                    );
