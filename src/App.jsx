...
const BASE_STOCKS = [
  { symbol:"PETR4", name:"Petrobras",     price:38.72, sector:"Energia" },
...
const BASE_FIIS = [
  { symbol:"MXRF11", name:"Maxi Renda",       price:10.24,  type:"CRI/CRA",  dy:11.2 },
  { symbol:"HGLG11", name:"CSHG Logística",   price:162.50, type:"Logística",dy:8.4  },
  { symbol:"KNRI11", name:"Kinea Renda",       price:148.30, type:"Híbrido",  dy:9.1  },
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"auto auto auto auto",alignContent:"start" }}>
          ...
          <div style={{ ...card,gridColumn:"4/5",gridRow:"2" }}>
            ...
            {/* cabeçalho da tabela */}
            <div style={{ display:"grid",gridTemplateColumns:"44px 1fr 60px 50px",gap:4,padding:"0 0 5px 0",borderBottom:`1px solid ${t.border}`,marginBottom:4 }}>
              {["Ticker","Nome","Preço","Var%"].map(h=><span key={h} style={{ fontSize:9,color:t.textMuted,fontWeight:700,textTransform:"uppercase",letterSpacing:0.6 }}>{h}</span>)}
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:2,maxHeight:175,overflowY:"auto" }}>
              {stocks.map(s=>{ const up=s.regularMarketChangePercent>=0; return (
                <div key={s.symbol} style={{ display:"grid",gridTemplateColumns:"44px 1fr 60px 50px",gap:4,padding:"5px 0",borderBottom:`1px solid ${t.border}44`,alignItems:"center" }}>
                  <span style={{ fontWeight:800,fontSize:11,color:t.text }}>{s.symbol}</span>
          ...
          {/* FIIs — lista profissional */}
          <div style={{ ...card,gridColumn:"4/5",gridRow:"3" }}>
            <div style={{ fontWeight:700,fontSize:13,marginBottom:10 }}>🏢 Fundos Imobiliários</div>
            <div style={{ display:"grid",gridTemplateColumns:"50px 1fr 55px 42px",gap:4,padding:"0 0 5px 0",borderBottom:`1px solid ${t.border}`,marginBottom:4 }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:2,maxHeight:160,overflowY:"auto" }}>
              {fiis.map(s=>{ const up=s.regularMarketChangePercent>=0; return (
                <div key={s.symbol} style={{ display:"grid",gridTemplateColumns:"50px 1fr 55px 42px",gap:4,padding:"5px 0",borderBottom:`1px solid ${t.border}44`,alignItems:"center" }}>
                  <span style={{ fontWeight:800,fontSize:11,color:t.text }}>{s.symbol}</span>
                  <span style={{ fontSize:10,color:t.textSub,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{s.type}</span>
                  <span style={{ fontSize:11,fontWeight:700,color:t.text }}>R${s.regularMarketPrice.toFixed(2)}</span>
