  ...
  const handle=async()=>{
    ...
    try {
      if(mode==="register"){
        ...
        const existing = await supa("GET", `nexus_users?username=eq.${encodeURIComponent(username)}&select=username`);
        if(existing && existing.length>0){setError("Usuário já existe.");return;}
        const dn = displayName.trim()||username;
        const ok = await dbSaveUser(username, password, dn);
        if(!ok){setError("Erro ao criar conta. Tente novamente.");return;}
        saveSession(username);
        onLogin(username, dn);
      } else {
        // Fetch specific user only
        const rows = await supa("GET", `nexus_users?username=eq.${encodeURIComponent(username)}&select=username,password,display_name`);
        if(!rows || rows.length===0){setError("Usuário não encontrado.");return;}
        const u = rows[0];
