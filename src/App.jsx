...
function Dashboard({ user, displayName, onLogout }) {
  ...
  const balance=carryover+totalInc-totalExp-totalSavings;
  ...
  useEffect(()=>{ 
    if(dbLoading||monthLoading) return;
    const t=setTimeout(()=>dbSaveMonth(user,viewYear,viewMonth,monthData),1000); 
    return ()=>clearTimeout(t); 
  },[monthData,user,dbLoading,monthLoading]);

  useEffect(()=>{ 
    if(dbLoading) return;
    const t=setTimeout(()=>dbSaveGlobal(user,{...global,savingsGoal}),1000); 
    return ()=>clearTimeout(t); 
  ...
  return (
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"repeat(6,minmax(150px,1fr))",alignContent:"start" }}>
          ...
          {/* BUDGET PLANNER */}
          <div style={{ ...card,gridColumn:"2/3",gridRow:"6" }}>
          ...
          {/* RESUMO DO MÊS */}
          <div style={{ ...card,gridColumn:"3/4",gridRow:"6" }}>
          ...
          {/* TOP GASTOS */}
          <div style={{ ...card,gridColumn:"4/5",gridRow:"6" }}>
          ...
          {/* SCORE FINANCEIRO */}
          <div style={{ ...card,gridColumn:"5",gridRow:"6" }}>
