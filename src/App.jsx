...
// ─── SUPABASE CLIENT (official SDK via CDN) ─────────────────────────────────
// SDK is loaded in index.html via script tag
...
// Initialize Supabase client using the global from CDN
const _supa = window.supabase.createClient(SUPA_URL, SUPA_KEY);

// ── Auth ──────────────────────────────────────────────────────────────────────
async function dbFindUser(username) {
  const { data, error } = await _supa.from("nexus_users")
...
async function dbSaveUser(username, password, displayName) {
  const { error } = await _supa.from("nexus_users")
...
async function dbLoadGlobal(username) {
  const { data } = await _supa.from("nexus_global")
    .select("data").eq("username", username).single();
  return data?.data || null;
}
async function dbSaveGlobal(username, data) {
  await _supa.from("nexus_global")
    .upsert({ username, data, updated_at: new Date().toISOString() }, { onConflict: "username" });
}

// ── Month data ────────────────────────────────────────────────────────────────
async function dbLoadMonth(username, year, month) {
  const { data } = await _supa.from("nexus_months")
    .select("data").eq("username", username).eq("year", year).eq("month", month).single();
  return data?.data || null;
}
async function dbSaveMonth(username, year, month, data) {
  await _supa.from("nexus_months")
    .upsert({ username, year, month, data, updated_at: new Date().toISOString() }, { onConflict: "username,year,month" });
}
async function dbLoadAllMonths(username, year) {
  const { data } = await _supa.from("nexus_months")
    .select("month,data").eq("username", username).eq("year", year);
  return data || [];
}

// ── Meta ──────────────────────────────────────────────────────────────────────
async function dbLoadMeta(username) {
  const { data } = await _supa.from("nexus_meta")
    .select("data").eq("username", username).single();
  return data?.data || null;
}
async function dbSaveMeta(username, data) {
  await _supa.from("nexus_meta")
    .upsert({ username, data }, { onConflict: "username" });
}

// ── Shared assets ─────────────────────────────────────────────────────────────
async function dbLoadSharedAssets() {
  const { data } = await _supa.from("nexus_shared_assets").select("symbol,data");
  return data || [];
}
async function dbSaveSharedAsset(symbol, data) {
  await _supa.from("nexus_shared_assets")
    .upsert({ symbol, data }, { onConflict: "symbol" });
}
...
const TRANSLATIONS = {
  "pt-BR": {
    balance:tr.balance, income:tr.income, expense:tr.expense, invested:tr.invested,
    savings:tr.savings, savingsRate:"Taxa de Economia", categories:"Categorias",
    wallet:"Carteira", totalInvested:"Total Investido", noInvestments:"Sem investimentos",
    barChart:"Receitas vs Gastos", monthly:tr.monthly, byCategory:"Por Categoria",
    noCategoryExpenses:"{tr.noCategoryExpenses}", calendar:"Calendário",
    stocks:"Ações B3", fiis:"FIIs", searchStock:tr.searchStock, searchFii:tr.searchFii,
    annualEvolution:"Evolução Anual", investments:"Investimentos", today:"Hoje",
    todayExpenses:"Gastos hoje", todayIncomes:"Receitas hoje", daysLeft:"Dias restantes",
    insightsTitle:"Insights do Mês", goals:"Metas", noGoals:"Nenhuma meta ainda.
Defina uma meta financeira!",
    addGoalValue:"{tr.addGoalValue}", liveExchange:"Câmbio Ao Vivo", update:"Atualizar",
    updatedAt:tr.updatedAt, switchTo:"trocar →", positive:tr.positive, negative:tr.negative,
    launches:"lançamentos", positions:"posições", deposits:"depósitos",
    monthGoal:tr.monthGoal, define:"Definir", newBtn:"+ Novo", newFem:"+ Nova",
    noExpenses:tr.noExpenses, noIncomes:tr.noIncomes,
    expensesTitle:"Gastos", incomesTitle:"Receitas", days:"dias",
  },
  "en-US": {
    balance:"Monthly Balance", income:"Income", expense:"Expenses", invested:"Invested",
    savings:"Savings", savingsRate:"Savings Rate", categories:"Categories",
    wallet:"Portfolio", totalInvested:"Total Invested", noInvestments:"No investments",
    barChart:"Income vs Expenses", monthly:"Monthly", byCategory:"By Category",
    noCategoryExpenses:"No categorized expenses", calendar:"Calendar",
    stocks:"B3 Stocks", fiis:"REITs", searchStock:"Search stock...", searchFii:"Search REIT...",
    annualEvolution:"Annual Evolution", investments:"Investments", today:"Today",
    todayExpenses:"Today's expenses", todayIncomes:"Today's income", daysLeft:"Days left",
    insightsTitle:"Monthly Insights", goals:"Goals", noGoals:"No goals yet.
Set a financial goal!",
    addGoalValue:"+ Add value", liveExchange:"Live Exchange", update:"Update",
    updatedAt:"Updated:", switchTo:"switch →", positive:"▲ Positive", negative:"▼ Negative",
    launches:"entries", positions:"positions", deposits:"deposits",
    monthGoal:"Monthly goal", define:"Set", newBtn:"+ New", newFem:"+ New",
    noExpenses:"No expenses this month", noIncomes:"No income this month",
    expensesTitle:"Expenses", incomesTitle:"Income", days:"days",
  },
  "de-DE": {
    balance:"Monatssaldo", income:"Einnahmen", expense:"Ausgaben", invested:"Investiert",
    savings:"Ersparnisse", savingsRate:"Sparquote", categories:"Kategorien",
    wallet:"Portfolio", totalInvested:"Gesamt investiert", noInvestments:"Keine Investitionen",
    barChart:"Einnahmen vs Ausgaben", monthly:"Monatlich", byCategory:"Nach Kategorie",
    noCategoryExpenses:"Keine kategorisierten Ausgaben", calendar:"Kalender",
    stocks:"Aktien", fiis:"Fonds", searchStock:"Aktie suchen...", searchFii:"Fonds suchen...",
    annualEvolution:"Jahresentwicklung", investments:"Investitionen", today:"Heute",
    todayExpenses:"Ausgaben heute", todayIncomes:"Einnahmen heute", daysLeft:"Verbleibende Tage",
    insightsTitle:"Monatseinblicke", goals:"Ziele", noGoals:"Noch keine Ziele.
Setze ein Finanzziel!",
    addGoalValue:"+ Wert hinzufügen", liveExchange:"Live-Wechselkurs", update:"Aktualisieren",
    updatedAt:"Aktualisiert:", switchTo:"wechseln →", positive:"▲ Positiv", negative:"▼ Negativ",
    launches:"Einträge", positions:"Positionen", deposits:"Einlagen",
    monthGoal:"Monatsziel", define:"Festlegen", newBtn:"+ Neu", newFem:"+ Neu",
    noExpenses:"Keine Ausgaben diesen Monat", noIncomes:"Keine Einnahmen diesen Monat",
    expensesTitle:"Ausgaben", incomesTitle:"Einnahmen", days:"Tage",
  },
  "fr-FR": {
    balance:"Solde Mensuel", income:"Revenus", expense:"Dépenses", invested:"Investi",
    savings:"Épargne", savingsRate:"Taux d'épargne", categories:"Catégories",
    wallet:"Portefeuille", totalInvested:"Total investi", noInvestments:"Aucun investissement",
    barChart:"Revenus vs Dépenses", monthly:"Mensuel", byCategory:"Par catégorie",
    noCategoryExpenses:"Aucune dépense catégorisée", calendar:"Calendrier",
    stocks:"Actions", fiis:"Fonds", searchStock:"Chercher action...", searchFii:"Chercher fonds...",
    annualEvolution:"Évolution annuelle", investments:"Investissements", today:"Aujourd'hui",
    todayExpenses:"Dépenses aujourd'hui", todayIncomes:"Revenus aujourd'hui", daysLeft:"Jours restants",
    insightsTitle:"Aperçus du mois", goals:"Objectifs", noGoals:"Aucun objectif.
Définissez un objectif!",
    addGoalValue:"+ Ajouter valeur", liveExchange:"Taux en direct", update:"Actualiser",
    updatedAt:"Mis à jour:", switchTo:"changer →", positive:"▲ Positif", negative:"▼ Négatif",
    launches:"entrées", positions:"positions", deposits:"dépôts",
    monthGoal:"Objectif mensuel", define:"Définir", newBtn:"+ Nouveau", newFem:"+ Nouvelle",
    noExpenses:"Aucune dépense ce mois", noIncomes:"Aucun revenu ce mois",
    expensesTitle:"Dépenses", incomesTitle:"Revenus", days:"jours",
  },
  "es-ES": {
    balance:"Saldo Mensual", income:"Ingresos", expense:tr.expense, invested:"Invertido",
    savings:"Ahorros", savingsRate:"Tasa de ahorro", categories:"Categorías",
    wallet:"Cartera", totalInvested:"Total invertido", noInvestments:"Sin inversiones",
    barChart:"Ingresos vs Gastos", monthly:"Mensual", byCategory:"Por categoría",
    noCategoryExpenses:"Sin gastos categorizados", calendar:"Calendario",
    stocks:"Acciones", fiis:"Fondos", searchStock:"Buscar acción...", searchFii:"Buscar fondo...",
    annualEvolution:"Evolución anual", investments:"Inversiones", today:"Hoy",
    todayExpenses:"Gastos hoy", todayIncomes:"Ingresos hoy", daysLeft:"Días restantes",
    insightsTitle:"Perspectivas del mes", goals:"Metas", noGoals:"Sin metas.
¡Define una meta!",
    addGoalValue:"+ Agregar valor", liveExchange:"Tipo de cambio", update:"Actualizar",
    updatedAt:"Actualizado:", switchTo:"cambiar →", positive:tr.positive, negative:tr.negative,
    launches:"entradas", positions:"posiciones", deposits:"depósitos",
    monthGoal:"Meta mensual", define:"Definir", newBtn:"+ Nuevo", newFem:"+ Nueva",
    noExpenses:"Sin gastos este mes", noIncomes:"Sin ingresos este mes",
    expensesTitle:tr.expense, incomesTitle:"Ingresos", days:"días",
  },
  "it-IT": {
    balance:"Saldo Mensile", income:"Entrate", expense:"Uscite", invested:"Investito",
    savings:"Risparmi", savingsRate:"Tasso di risparmio", categories:"Categorie",
    wallet:"Portafoglio", totalInvested:"Totale investito", noInvestments:"Nessun investimento",
    barChart:"Entrate vs Uscite", monthly:"Mensile", byCategory:"Per categoria",
    noCategoryExpenses:"Nessuna uscita categorizzata", calendar:"Calendario",
    stocks:"Azioni", fiis:"Fondi", searchStock:"Cerca azione...", searchFii:"Cerca fondo...",
    annualEvolution:"Evoluzione annuale", investments:"Investimenti", today:"Oggi",
    todayExpenses:"Uscite oggi", todayIncomes:"Entrate oggi", daysLeft:"Giorni rimasti",
    insightsTitle:"Approfondimenti del mese", goals:"Obiettivi", noGoals:"Nessun obiettivo.
Imposta un obiettivo!",
    addGoalValue:"+ Aggiungi valore", liveExchange:"Cambio in tempo reale", update:"Aggiorna",
    updatedAt:"Aggiornato:", switchTo:"cambia →", positive:tr.positive, negative:tr.negative,
    launches:"voci", positions:"posizioni", deposits:"depositi",
    monthGoal:"Obiettivo mensile", define:"Imposta", newBtn:"+ Nuovo", newFem:"+ Nuova",
    noExpenses:"Nessuna uscita questo mese", noIncomes:"Nessuna entrata questo mese",
    expensesTitle:"Uscite", incomesTitle:"Entrate", days:"giorni",
  },
  "pt-PT": {
    balance:"Saldo Mensal", income:tr.income, expense:"Despesas", invested:tr.invested,
    savings:tr.savings, savingsRate:"Taxa de poupança", categories:"Categorias",
    wallet:"Carteira", totalInvested:"Total investido", noInvestments:"Sem investimentos",
    barChart:"Receitas vs Despesas", monthly:tr.monthly, byCategory:"Por categoria",
    noCategoryExpenses:"Sem despesas categorizadas", calendar:"Calendário",
    stocks:"Ações", fiis:"Fundos", searchStock:"Pesquisar ação...", searchFii:"Pesquisar fundo...",
    annualEvolution:"Evolução anual", investments:"Investimentos", today:"Hoje",
    todayExpenses:"Despesas hoje", todayIncomes:"Receitas hoje", daysLeft:"Dias restantes",
    insightsTitle:"Insights do mês", goals:"Metas", noGoals:"Sem metas.
Define uma meta!",
    addGoalValue:"{tr.addGoalValue}", liveExchange:"Câmbio ao vivo", update:"Atualizar",
    updatedAt:tr.updatedAt, switchTo:"trocar →", positive:tr.positive, negative:tr.negative,
    launches:"lançamentos", positions:"posições", deposits:"depósitos",
    monthGoal:tr.monthGoal, define:"Definir", newBtn:"+ Novo", newFem:"+ Nova",
    noExpenses:"Sem despesas este mês", noIncomes:"Sem receitas este mês",
    expensesTitle:"Despesas", incomesTitle:"Receitas", days:"dias",
  },
};
// Fallback to pt-BR for unlisted languages
function getTr(language) {
  return TRANSLATIONS[language] || TRANSLATIONS["en-US"];
}

// ─── EXCHANGE RATES (cache 1h in localStorage) ───────────────────────────────
async function getExchangeRates() {
  try {
    const cached = JSON.parse(localStorage.getItem("nexus_rates")||"{}");
    const age = Date.now() - (cached.ts||0);
    if(cached.rates && age < 3600000) return cached.rates; // 1h cache
    ...
    return { BRL:1, USD:0.18, EUR:0.17 };
  }
}
...
function Dashboard({ user, displayName, onLogout }) {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try {
        ...
        if(g) {
          setGlobalState({ categories:g.categories||DEFAULT_CATEGORIES, investments:g.investments||[], savings:g.savings||[], goals:g.goals||[], theme:g.theme||"Dark Red", currency:g.currency||"BRL", language:g.language||"pt-BR", customThemes:g.customThemes||[] });
  ...
  const _customTheme = (global.customThemes||[]).find(ct=>ct.name===global.theme);
  const _baseTheme = THEMES[global.theme]||THEMES["Dark Red"];
  const t = _customTheme ? {..._baseTheme, ..._customTheme} : _baseTheme;
  ...
  const sym=CURRENCIES[global.currency]?.symbol||"R$";
  const rate = rates[global.currency]||1;
  const tr = getTr(global.language||"pt-BR");
  const fmt=v=>{
    const converted = Number(v||0) * rate;
    const locale = global.language||"pt-BR";  // language drives number formatting
    return `${sym} ${converted.toLocaleString(locale,{minimumFractionDigits:2,maximumFractionDigits:2})}`;
  };
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const cache = {};
      rows.forEach(r=>{ if(r.data) cache[r.month]=r.data; });
      // Always include current month in cache
      cache[viewMonth] = monthData;
  ...
  const monthly=MONTHS_SHORT.map((m,i)=>{
    ...
    const R = (d.incomes||[]).reduce((s,x)=>s+Number(x.value||0),0) * rate;
    const G = (d.expenses||[]).reduce((s,x)=>s+Number(x.value||0),0) * rate;
    return {m,R,G};
  });
  const expByCat=categories.map(c=>({name:c.name,value:expenses.filter(e=>e.catId===c.id).reduce((s,e)=>s+Number(e.value||0),0)*rate,color:c.color,icon:c.icon})).filter(c=>c.value>0);
  ...
  return (
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ background:t.header,borderBottom:`1px solid ${t.border}`,padding:"0 18px",height:54,display:"flex",alignItems:"center",gap:10,flexShrink:0,position:"sticky",top:0,zIndex:200 }}>
        ...
        <button onClick={()=>setShowSettings(true)} style={{ ...Gb(t.textSub),padding:"5px 10px",fontSize:16 }}>⚙</button>
        ...
        <div style={{ display:"flex",gap:10,alignItems:"flex-start" }}>
          ...
          <div style={{ width:200,flexShrink:0,display:"flex",flexDirection:"column",gap:9 }}>
            ...
            {[
              ...
              { label:tr.savings,      val:totalSavings, color:t.yellow, sub:`${global.savings.length} ${tr.deposits}`, mo:null },
            ].map(k=>(
          ...
          <button onClick={()=>setShowSavings(true)} style={{ background:t.yellow,border:"none",borderRadius:8,color:"#000",padding:"8px 10px",cursor:"pointer",fontSize:11,fontWeight:800,display:"flex",alignItems:"center",gap:6,justifyContent:"center",width:"100%" }}>
            {tr.savings} · {fmt(global.savings.reduce((s,e)=>s+Number(e.value||0),0))}
          </button>
            ...
            <div style={{ ...card, flex:1, display:"flex", flexDirection:"column", minHeight:0 }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6, overflowY:"auto", flex:1 }}>
                {(global.goals||[]).length===0 ? (
                  <div style={{ color:t.textMuted, fontSize:11, textAlign:"center", padding:"12px 0" }}>
                  ...
                  const done = pct >= 100;
                  return (
                    <div key={goal.id} style={{ padding:"7px 8px", borderRadius:8, background:t.bgCard2 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        <span style={{ fontSize:11, color:t.text, fontWeight:600 }}>{goal.icon} {goal.name}</span>
                        <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                          {done && <span style={{ fontSize:9, color:t.green }}>✓</span>}
                          <button onClick={()=>updGlobal(p=>({...p,goals:(p.goals||[]).filter(g=>g.id!==goal.id)}))} style={{ background:"none",border:"none",cursor:"pointer",color:t.textMuted,padding:0,lineHeight:0 }}>{Ico.trash}</button>
                        </div>
                      </div>
                      <div style={{ display:"flex", justifyContent:"space-between", marginTop:4 }}>
                        <span style={{ fontSize:9, color:t.textSub }}>{fmt(goal.current)} de {fmt(goal.target)}</span>
                        <span style={{ fontSize:9, color:done?t.green:t.accent }}>{pct.toFixed(0)}%</span>
                      </div>
                      <div style={{ background:t.border, borderRadius:3, height:4, marginTop:4, overflow:"hidden" }}>
                        <div style={{ width:`${pct}%`, height:"100%", background:done?t.green:t.accent, borderRadius:3, transition:"width 0.5s" }}/>
                      </div>
                      {/* botão adicionar valor */}
                      {!done && (
                        <button onClick={()=>{ const v=prompt("Quanto adicionar à meta?"); if(v&&!isNaN(v)) updGlobal(p=>({...p,goals:(p.goals||[]).map(g=>g.id===goal.id?{...g,current:Math.min(Number(g.current)+Number(v),g.target)}:g)})); }}
                          style={{ ...Gb(t.accent), fontSize:9, padding:"2px 8px", marginTop:5, width:"100%", justifyContent:"center" }}>
                          {tr.addGoalValue}
                        </button>
                      )}
                    </div>
          ...
          {/* CÂMBIO AO VIVO */}
            <div style={{ ...card }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                ...
                <button onClick={refreshRates} style={{ ...Gb(t.accent), padding:"3px 7px", fontSize:10, display:"flex", alignItems:"center", gap:4 }}>
                  {loadingRates ? "⏳" : tr.update}
                </button>
              </div>
              {ratesUpdatedAt && (
                <div style={{ fontSize:9, color:t.textMuted, marginBottom:8 }}>
                  Atualizado: {ratesUpdatedAt.toLocaleTimeString(global.language||"pt-BR")}
                </div>
              )}
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {[
                  { code:"USD", name:"Dólar", flag:"🇺🇸", sym:"US$" },
                  { code:"EUR", name:"Euro",  flag:"🇪🇺", sym:"€"  },
                  { code:"BRL", name:"Real",  flag:"🇧🇷", sym:"R$" },
                ].filter(c => c.code !== global.currency).map(c => {
                  // Convert 1 unit of OTHER currency INTO current currency
                  // rates are: how many USD = 1 BRL, how many EUR = 1 BRL
                  // So: 1 BRL in USD = rates.USD, 1 BRL in EUR = rates.EUR
                  // We want: 1 [other] = X [current]
                  const curSym = CURRENCIES[global.currency]?.symbol||"";
                  let valueIn1; // value of 1 unit of c.code in current currency
                  if(global.currency==="BRL") {
                    // 1 USD in BRL = 1/rates.USD
                    // 1 EUR in BRL = 1/rates.EUR
                    valueIn1 = c.code==="USD" ? (1/rates.USD) : (1/rates.EUR);
                  } else if(global.currency==="USD") {
                    // 1 BRL in USD = rates.USD
                    // 1 EUR in USD = rates.EUR/rates.USD (both relative to BRL)
                    valueIn1 = c.code==="BRL" ? rates.USD : (rates.EUR/rates.USD);
                  } else { // EUR
                    // 1 BRL in EUR = rates.EUR
                    // 1 USD in EUR = rates.USD/rates.EUR
                    valueIn1 = c.code==="BRL" ? rates.EUR : (rates.USD/rates.EUR);
                  }
                  const formatted = valueIn1 ? valueIn1.toLocaleString("pt-BR",{minimumFractionDigits:4,maximumFractionDigits:4}) : "—";
                  return (
                    <div key={c.code} onClick={()=>updGlobal(p=>({...p,currency:c.code}))}
                      style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px", borderRadius:8, background:t.bgCard2, cursor:"pointer", transition:"background 0.15s" }}
                      onMouseEnter={e=>e.currentTarget.style.background=t.border}
                      onMouseLeave={e=>e.currentTarget.style.background=t.bgCard2}>
                      <span style={{ fontSize:18 }}>{c.flag}</span>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:12, color:t.text, fontWeight:700 }}>{c.name} <span style={{ color:t.textMuted, fontWeight:400 }}>({c.code})</span></div>
                        <div style={{ fontSize:13, color:t.accent, fontWeight:800, marginTop:2 }}>
                          1 {c.code} = {curSym} {formatted}
                        </div>
                      </div>
                      <div style={{ fontSize:9, color:t.textMuted, textAlign:"center" }}>{tr.switchTo}</div>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ ...card,gridColumn:"1/3",gridRow:"1" }}>
            ...
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={monthly} barGap={2} barSize={9}>
                ...
                <YAxis tick={{ fill:t.textMuted,fontSize:9 }} axisLine={false} tickLine={false} width={40} tickFormatter={v=>v>0?`${sym}${(v/1000).toFixed(0)}k`:0}/>
          ...
          <div style={{ ...card,gridColumn:"3",gridRow:"1" }}>
            <div style={{ fontWeight:700,fontSize:13,marginBottom:10 }}>{tr.byCategory}</div>
            {expByCat.length===0
              ? <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"30px 0",gap:8 }}>
                  <span style={{ fontSize:28 }}>📊</span>
                  <span style={{ color:t.textMuted,fontSize:11 }}>{tr.noCategoryExpenses}</span>
                </div>
              : <div style={{ display:"flex",flexDirection:"column",gap:7 }}>
                  {expByCat.sort((a,b)=>b.value-a.value).map((cat,i)=>{
                    const max = expByCat[0].value;
                    ...
                    return (
                      <div key={i}>
                        ...
                        <div style={{ background:t.bgCard2,borderRadius:4,height:6,overflow:"hidden" }}>
          ...
          <div style={{ ...card,gridColumn:"4",gridRow:"1/4" }}>
            ...
            {/* {tr.totalInvested?.toUpperCase()} destacado */}
          ...
          <div style={{ ...card,gridColumn:"1/2",gridRow:"2" }}>
            ...
            {filteredExpenses.length===0
              ? <div style={{ color:t.textMuted,fontSize:12,textAlign:"center",padding:"16px 0" }}>{searchQuery?tr.noExpenses:tr.noExpenses}</div>
          ...
          <div style={{ ...card,gridColumn:"2/3",gridRow:"2" }}>
            ...
            {filteredIncomes.length===0
              ? <div style={{ color:t.textMuted,fontSize:12,textAlign:"center",padding:"16px 0" }}>{searchQuery?tr.noExpenses:tr.noIncomes}</div>
              : <div style={{ display:"flex",flexDirection:"column",maxHeight:220,overflowY:"auto" }}>
          ...
          {/* INVESTIMENTOS LISTA */}
          <div style={{ ...card,gridColumn:"1/4",gridRow:"5" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9 }}>
          ...
          <InsightsCard
            ...
            monthlyCache={monthlyCache} card={card}
          />
          ...
          <div style={{ ...card,gridColumn:"4",gridRow:"4",borderLeft:`3px solid ${t.yellow}`,display:"flex",flexDirection:"column",gap:6 }}>
            ...
            <div style={{ fontSize:11,color:t.textSub }}>{now.toLocaleDateString(global.language||"pt-BR",{weekday:"long",day:"2-digit",month:"long"})}</div>
      ...
      <Modal open={modal==="edit_expense"} onClose={closeModal} title="Editar Gasto" t={t}>
        ...
        <button onClick={saveEdit} style={{ ...Cb(t.red),width:"100%",justifyContent:"center",padding:"11px",fontSize:13 }}>Salvar Gasto</button>
      </Modal>
      ...
      <Modal open={modal==="edit_income"} onClose={closeModal} title="Editar Receita" t={t}>
        ...
        <button onClick={saveEdit} style={{ ...Cb(t.green),width:"100%",justifyContent:"center",padding:"11px",fontSize:13 }}>Salvar Receita</button>
      </Modal>

      <Modal open={modal==="edit_investment"} onClose={closeModal} title="Editar Investimento" t={t}>
        <input style={inp} placeholder="Nome *" value={form.name||""} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/>
        <input style={inp} placeholder="Ticker (ex: ITUB4)" value={form.ticker||""} onChange={e=>setForm(p=>({...p,ticker:e.target.value}))}/>
        <input style={inp} type="number" placeholder="Valor *" value={form.value||""} onChange={e=>setForm(p=>({...p,value:e.target.value}))}/>
        <input style={inp} placeholder="Tipo (Ação, FII, etc)" value={form.type||""} onChange={e=>setForm(p=>({...p,type:e.target.value}))}/>
        <button onClick={saveEdit} style={{ ...Cb(t.accent),width:"100%",justifyContent:"center",padding:"11px",fontSize:13 }}>Salvar Investimento</button>
      </Modal>

      <Modal open={modal==="edit_saving"} onClose={closeModal} title="Editar Poupança" t={t}>
        <input style={inp} placeholder="Descrição *" value={form.desc||""} onChange={e=>setForm(p=>({...p,desc:e.target.value}))}/>
        <input style={inp} type="number" placeholder="Valor *" value={form.value||""} onChange={e=>setForm(p=>({...p,value:e.target.value}))}/>
        <button onClick={saveEdit} style={{ ...Cb(t.yellow,"#000"),width:"100%",justifyContent:"center",padding:"11px",fontSize:13 }}>Salvar Poupança</button>
      </Modal>

      <Modal open={modal==="edit_category"} onClose={closeModal} title="Editar Categoria" t={t}>
        <input style={inp} placeholder="Nome *" value={form.name||""} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/>
        <div style={{ display:"flex",gap:10 }}>
          <input style={{...inp,flex:1}} placeholder="Emoji (ex: 🍔)" value={form.icon||""} onChange={e=>setForm(p=>({...p,icon:e.target.value}))}/>
          <div style={{ display:"flex",alignItems:"center",gap:6 }}>
            <span style={{ color:t.textMuted,fontSize:12 }}>Cor:</span>
            <input type="color" value={form.color||"#888"} onChange={e=>setForm(p=>({...p,color:e.target.value}))} style={{ width:36,height:36,borderRadius:8,border:"none",cursor:"pointer" }}/>
          </div>
        </div>
        <button onClick={saveEdit} style={{ ...Cb(t.accent),width:"100%",justifyContent:"center",padding:"11px",fontSize:13 }}>Salvar Categoria</button>
      </Modal>

      {/* META NOVA MODAL */}
      <Modal open={modal==="goal_new"} onClose={closeModal} title="Nova Meta Financeira" t={t}>
        <div style={{ color:t.textSub, fontSize:12, marginBottom:14 }}>
          Ex: Viagem, Carro novo, Reserva de emergência...
        </div>
        ...
        <button onClick={()=>{
          ...
          updGlobal(p=>({...p, goals:[...(p.goals||[]), {id:Date.now(), name:form.name, icon:form.icon||"🎯", target:Number(form.target), current:Number(form.current||0)}]}));
          closeModal();
        }} style={{ ...Cb(t.accent), width:"100%", justifyContent:"center", padding:"11px", fontSize:13, marginTop:4 }}>
          Criar Meta
        </button>
      </Modal>
...
// ─── INSIGHTS CARD ───────────────────────────────────────────────────────────
function InsightsCard({ expenses, incomes, investments, categories, monthData, viewMonth, viewYear, now, t, fmt, MONTH_NAMES, monthlyCache, card }) {
  const totalExp = expenses.reduce((s,e)=>s+Number(e.value||0),0);
  const totalInc = incomes.reduce((s,e)=>s+Number(e.value||0),0);
  const totalInv = investments.reduce((s,e)=>s+Number(e.value||0),0);
  ...
  return (
    <div style={{ ...card }}>
      <div style={{ fontWeight:700, fontSize:13, marginBottom:10 }}>
        📅 {tr.calendar} — {MONTH_NAMES[viewMonth]} {viewYear}
      </div>
      ...
      <div className="fade-in" onClick={e=>e.stopPropagation()} style={{ background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:18,padding:24,width:420,maxHeight:"85vh",display:"flex",flexDirection:"column",boxShadow:`0 0 80px ${t.accentGlow}` }}>
        ...
        {tab==="language" && (
          <div style={{ overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:6 }}>
            ...
            {LANGUAGES.map(lang=>(
              <div key={lang.code} onClick={()=>updGlobal(p=>({...p,language:lang.code,currency:lang.currency}))}
                style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,cursor:"pointer",
                  background: g.language===lang.code ? t.accent+"22" : t.bgCard2,
                  border: `1px solid ${g.language===lang.code ? t.accent : t.border}`,
                ...
                {g.language===lang.code && <span style={{ color:t.accent,fontSize:14 }}>✓</span>}
              </div>
            ))}
          </div>
        )}
