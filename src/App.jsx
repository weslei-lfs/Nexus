...
// ─── MARKET DATA (simulado realista, varia a cada atualização) ────────────────
const BASE_STOCKS = [
  { symbol:"PETR4", name:"Petrobras",     price:38.72 },
  { symbol:"VALE3", name:"Vale",           price:61.45 },
  { symbol:"ITUB4", name:"Itaú Unibanco", price:34.18 },
  { symbol:"BBDC4", name:"Bradesco",       price:14.93 },
  { symbol:"ABEV3", name:"Ambev",          price:11.87 },
  { symbol:"WEGE3", name:"WEG",            price:52.34 },
];
const BASE_FIIS = [
  { symbol:"MXRF11", name:"Maxi Renda",       price:10.24  },
  { symbol:"HGLG11", name:"CSHG Logística",   price:162.50 },
  { symbol:"KNRI11", name:"Kinea Renda",       price:148.30 },
  { symbol:"XPML11", name:"XP Malls",          price:95.60  },
];
const genMarket = (base) => base.map(item => {
  ...
  return {
    symbol: item.symbol,
    longName: item.name,
    regularMarketPrice: +(item.price * (1 + change / 100)).toFixed(2),
    regularMarketChangePercent: +change.toFixed(2),
  };
});
...
function loadGlobal(user) {
  try { return JSON.parse(localStorage.getItem(globalKey(user))) || {}; } catch { return {}; }
}
...
function loadMeta(user) {
  try { return JSON.parse(localStorage.getItem(metaKey(user))); } catch { return null; }
}
function saveMeta(user, data) { localStorage.setItem(metaKey(user), JSON.stringify(data)); }

function getCarryover(user, year, month) {
  const meta = loadMeta(user);
  if (!meta) return 0;
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#0d0d0d",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <div style={{ background:"#161616",border:"1px solid #242424",borderRadius:18,padding:40,width:380,boxShadow:"0 0 80px rgba(229,57,53,0.1)" }}>
        {/* Logo */}
        <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:32,justifyContent:"center" }}>
          <div style={{ width:36,height:36,borderRadius:10,background:"#e53935",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:18,color:"#fff",boxShadow:"0 0 20px rgba(229,57,53,0.4)" }}>N</div>
          <span style={{ fontWeight:800,fontSize:22,color:"#fff",letterSpacing:-0.5 }}>Nexus</span>
        </div>

        <div style={{ display:"flex",background:"#111",borderRadius:9,overflow:"hidden",marginBottom:24 }}>
          {["login","register"].map(m=>(
            <button key={m} onClick={()=>{ setMode(m); setError(""); }} style={{ flex:1,padding:"9px",border:"none",cursor:"pointer",fontWeight:700,fontSize:12,background:mode===m?"#e53935":"transparent",color:mode===m?"#fff":"#888",transition:"all 0.2s",textTransform:"uppercase",letterSpacing:0.8 }}>
        ...
        {error && <div style={{ color:"#ff5252",fontSize:12,marginBottom:12,textAlign:"center" }}>{error}</div>}

        <button onClick={handle} style={{ width:"100%",background:"#e53935",border:"none",borderRadius:9,color:"#fff",padding:"12px",fontSize:14,fontWeight:800,cursor:"pointer",letterSpacing:0.3,boxShadow:"0 4px 20px rgba(229,57,53,0.3)" }}>
...
function Dashboard({ user, onLogout }) {
  ...
  const monthly = MONTHS_SHORT.map((m,i)=>{ const d=loadMonth(user,viewYear,i); return { m, R:d.incomes.reduce((s,x)=>s+Number(x.value||0),0), G:d.expenses.reduce((s,x)=>s+Number(x.value||0),0) }; });
  ...
  const addExpense  = ()=>{ if(!form.desc||!form.value)return; updMonth(p=>({...p,expenses:[...p.expenses,{...form,id:Date.now(),date:form.date||new Date().toISOString().slice(0,10)}]})); closeModal(); };
  const addIncome   = ()=>{ if(!form.desc||!form.value)return; updMonth(p=>({...p,incomes:[...p.incomes,{...form,id:Date.now(),date:form.date||new Date().toISOString().slice(0,10)}]})); closeModal(); };
    ...
    <div style={{ minHeight:"100vh", background:t.bg, color:t.text, fontFamily:"'DM Sans','Segoe UI',sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:t.header, borderBottom:`1px solid ${t.border}`, padding:"0 18px", height:54, display:"flex", alignItems:"center", gap:12, flexShrink:0, position:"sticky", top:0, zIndex:200 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:9, overflow:"hidden" }}>
          ...
          <div style={{ padding:"0 14px", minWidth:150, textAlign:"center" }}>
        ...
        <div style={{ display:"grid", gap:10,
                ...
                {categories.map(cat=>{
                  ...
                  return (
                    <div key={cat.id} style={{ display:"flex", alignItems:"center", gap:7, padding:"5px 7px", borderRadius:7, background:t.bgCard2 }}>
                      ...
                      <button onClick={()=>delCat(cat.id)} style={{ background:"none", border:"none", cursor:"pointer", color:t.textMuted, padding:0, lineHeight:0 }}>{Ico.trash}</button>
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ ...card, gridColumn:"5", gridRow:"1/4" }}>
            ...
            {investments.length===0
              ...
              : <>
                  <ResponsiveContainer width="100%" height={120}><PieChart><Pie data={investments.map(i=>({name:i.name,value:Number(i.value)}))} cx="50%" cy="50%" innerRadius={34} outerRadius={54} dataKey="value">{investments.map((_,i)=><Cell key={i} fill={t.chart[i%t.chart.length]}/>)}</Pie><Tooltip {...TT} formatter={v=>fmt(v)}/></PieChart></ResponsiveContainer>
            ...
            <button onClick={()=>setModal("investment")} style={{ ...Gb(t.border), width:"100%", justifyContent:"center", marginTop:10, color:t.textSub }}>{Ico.plus} Novo Investimento</button>
          </div>
          ...
          <div style={{ ...card, gridColumn:"4/5", gridRow:"2" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
              {stocks.map(s=>{ const up=s.regularMarketChangePercent>=0; return (
                <div key={s.symbol} style={{ background:t.bgCard2, borderRadius:7, padding:"7px 9px", borderLeft:`2px solid ${up?t.green:t.red}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    ...
                    <span style={{ fontSize:9, color:up?t.green:t.red, fontWeight:700 }}>{up?"▲":"▼"}{Math.abs(s.regularMarketChangePercent).toFixed(2)}%</span>
                  </div>
                  <div style={{ fontSize:13, fontWeight:800, marginTop:2 }}>R$ {s.regularMarketPrice.toFixed(2)}</div>
                </div>
              ); })}
            </div>
          </div>
          ...
          <div style={{ ...card, gridColumn:"4/5", gridRow:"3" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
              {fiis.map(s=>{ const up=s.regularMarketChangePercent>=0; return (
                <div key={s.symbol} style={{ background:t.bgCard2, borderRadius:7, padding:"7px 9px", borderLeft:`2px solid ${up?t.green:t.red}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <span style={{ fontWeight:800, fontSize:11 }}>{s.symbol}</span>
                    <span style={{ fontSize:9, color:up?t.green:t.red, fontWeight:700 }}>{up?"▲":"▼"}{Math.abs(s.regularMarketChangePercent).toFixed(2)}%</span>
      ...
      {showScrollTop && (
        <button onClick={()=>scrollRef.current?.scrollTo({ top:0, behavior:"smooth" })}
          style={{ position:"fixed", bottom:24, right:24, width:42, height:42, borderRadius:"50%", background:t.accent, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", boxShadow:`0 4px 20px ${t.accentGlow}`, zIndex:500, transition:"all 0.2s" }}>
          {Ico.arrowUp}
        </button>
      )}
      ...
      <Modal open={modal==="expense"} onClose={closeModal} title={`Novo Gasto — ${MONTH_NAMES[viewMonth]}`} t={t}>
        ...
        <button onClick={addExpense} style={{ ...Cb(t.red), width:"100%", justifyContent:"center", padding:"10px", fontSize:13, marginTop:4 }}>Adicionar Gasto</button>
      </Modal>
      ...
      <Modal open={modal==="income"} onClose={closeModal} title={`Nova Receita — ${MONTH_NAMES[viewMonth]}`} t={t}>
        ...
        <input style={inp} type="date" value={form.date||""} onChange={e=>setForm(p=>({...p,date:e.target.value}))}/>
        <button onClick={addIncome} style={{ ...Cb(t.green), width:"100%", justifyContent:"center", padding:"10px", fontSize:13, marginTop:4 }}>Adicionar Receita</button>
      </Modal>
      ...
      <Modal open={modal==="investment"} onClose={closeModal} title="Novo Investimento" t={t}>
        ...
        <button onClick={addInvest} style={{ ...Cb(t.accent), width:"100%", justifyContent:"center", padding:"10px", fontSize:13, marginTop:4 }}>Adicionar</button>
      </Modal>
      ...
      <Modal open={modal==="category"} onClose={closeModal} title="Nova Categoria" t={t}>
        ...
        <button onClick={addCat} style={{ ...Cb(t.accent), width:"100%", justifyContent:"center", padding:"10px", fontSize:13, marginTop:12 }}>Criar Categoria</button>
      </Modal>
    </div>
  );
}
