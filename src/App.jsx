...
function Dashboard({ user, displayName, onLogout }) {
  ...
  const fmt=v=>{
    ...
    const locale = global.language||"pt-BR";
            ...
            <div style={{ ...card }}>
              ...
              {ratesUpdatedAt && (
                <div style={{ fontSize:9, color:t.textMuted, marginBottom:8 }}>
                  Atualizado: {ratesUpdatedAt.toLocaleTimeString("pt-BR")}
                </div>
              )}
...
function SettingsModal({ open, onClose, global: g, updGlobal, t }) {
  ...
  const saveCustomTheme = () => {
    ...
    const newTheme = {
      ...
      accent: customAccent, accentGlow: customAccent+"66",
      bg: customBg, bgCard: customCard, bgCard2: customCard+"cc",
      text: customText, textSub: customText+"cc", textMuted: customText+"88",
      border: customAccent+"33", green:"#4caf50", red:"#f44336", yellow:"#ffb300",
      chart:[customAccent,"#4caf50","#2196f3","#ff9800","#9c27b0","#00bcd4"],
    };
    updGlobal(p => ({ ...p, customThemes: [...(p.customThemes||[]), newTheme], theme: customName.trim() }));
    setCustomName("");
    alert("Tema criado!");
  };
        ...
        {tab==="theme" && (
          <div style={{ overflowY:"auto",flex:1 }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12 }}>
              {[
                { label:"Cor principal (accent)", val:customAccent, set:setCustomAccent },
            ...
            {/* Preview */}
            <div style={{ background:customBg,borderRadius:10,padding:12,marginBottom:12,border:`1px solid ${customAccent}33` }}>
              <div style={{ color:customText,fontWeight:700,fontSize:12,marginBottom:8 }}>Preview</div>
              <div style={{ background:customCard,borderRadius:8,padding:10,border:`1px solid ${customAccent}33` }}>
                <div style={{ color:customAccent,fontSize:16,fontWeight:800 }}>R$ 5.000,00</div>
                <div style={{ color:customText+"88",fontSize:10,marginTop:2 }}>Saldo do Mês</div>
                <div style={{ height:4,background:customAccent,borderRadius:4,marginTop:8,width:"70%" }}/>
              </div>
            </div>
              ...
              <div style={{ marginTop:14 }}>
                ...
                <div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>
                  {(g.customThemes||[]).map(ct=>(
                    <div key={ct.name} onClick={()=>updGlobal(p=>({...p,theme:ct.name}))}
                      style={{ padding:"5px 10px",borderRadius:20,cursor:"pointer",fontSize:11,fontWeight:700,
