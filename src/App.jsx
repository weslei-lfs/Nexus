...
export default function App() {
  ...
  // ── market (dados reais via proxy, cache 15min) ──
  const BRAPI_TOKEN = "1gPHxuoP6T6aBWFbe2ryko";
  ...
  const fetchMarket = useCallback(async () => {
    // verifica cache
    try {
      const cached = JSON.parse(localStorage.getItem("nexus_market_cache") || "null");
    ...
    setLoadingMkt(true);
    try {
      const sUrl = `https://brapi.dev/api/quote/PETR4,VALE3,ITUB4,BBDC4,ABEV3,WEGE3?fundamental=false&token=${BRAPI_TOKEN}`;
      const fUrl = `https://brapi.dev/api/quote/MXRF11,HGLG11,KNRI11,XPML11?fundamental=false&token=${BRAPI_TOKEN}`;
      const [s, f] = await Promise.all([
        fetch(PROXY + encodeURIComponent(sUrl)),
        fetch(PROXY + encodeURIComponent(fUrl)),
      ]);
      const sd = await s.json();
      const fd = await f.json();
      if (sd.results && fd.results) {
        setStocks(sd.results);
        setFiis(fd.results);
        localStorage.setItem("nexus_market_cache", JSON.stringify({
          ts: Date.now(), stocks: sd.results, fiis: fd.results
        }));
      }
    } catch (e) {
      console.error("Erro ao buscar mercado:", e);
    }
    setLoadingMkt(false);
  }, []);
  ...
  useEffect(() => {
    fetchMarket();
    const iv = setInterval(fetchMarket, CACHE_MS);
