...
async function dbLoadUsers() {
  const rows = await supa("GET", "nexus_users?select=username,password,display_name");
  const map = {};
  (rows||[]).forEach(r => { map[r.username] = { password: r.password, displayName: r.display_name }; });
  return map;
}
async function dbSaveUser(username, password, displayName) {
  return supa("POST", "nexus_users", { username, password, display_name: displayName });
}

// ── Global data ───────────────────────────────────────────────────────────────
async function dbLoadGlobal(username) {
  const rows = await supa("GET", `nexus_global?username=eq.${username}&select=data`);
  return rows?.[0]?.data || null;
}
async function dbSaveGlobal(username, data) {
  return supa("POST", "nexus_global", { username, data, updated_at: new Date().toISOString() });
}

// ── Month data ────────────────────────────────────────────────────────────────
async function dbLoadMonth(username, year, month) {
  const rows = await supa("GET", `nexus_months?username=eq.${username}&year=eq.${year}&month=eq.${month}&select=data`);
  return rows?.[0]?.data || null;
}
async function dbSaveMonth(username, year, month, data) {
  return supa("POST", "nexus_months", { username, year, month, data, updated_at: new Date().toISOString() });
}
async function dbLoadAllMonths(username, year) {
  const rows = await supa("GET", `nexus_months?username=eq.${username}&year=eq.${year}&select=month,data`);
  return rows || [];
}

// ── Meta ──────────────────────────────────────────────────────────────────────
async function dbLoadMeta(username) {
  const rows = await supa("GET", `nexus_meta?username=eq.${username}&select=data`);
  return rows?.[0]?.data || null;
}
async function dbSaveMeta(username, data) {
  return supa("POST", "nexus_meta", { username, data });
}

// ── Shared assets ─────────────────────────────────────────────────────────────
async function dbLoadSharedAssets() {
  return supa("GET", "nexus_shared_assets?select=symbol,data") || [];
}
async function dbSaveSharedAsset(symbol, data) {
  return supa("POST", "nexus_shared_assets", { symbol, data });
}
...
function Dashboard({ user, displayName, onLogout }) {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try {
        ...
        if(g) setGlobalState({ categories:g.categories||DEFAULT_CATEGORIES, investments:g.investments||[], savings:g.savings||[], theme:g.theme||"Dark Red", currency:g.currency||"BRL" });
        // Load meta
  ...
  useEffect(()=>{
    if(loading) return;
    const t = setTimeout(()=>{ dbSaveGlobal(user,global); },800);
    return ()=>clearTimeout(t);
  },[global,user,loading]);
  ...
  return (
    ...
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"auto auto auto auto auto auto",alignContent:"start" }}>

          {/* COL 1 */}
          <div style={{ gridColumn:"1",gridRow:"1/7",minWidth:0,display:"flex",flexDirection:"column",gap:9 }}>
          ...
          {/* EVOLUÇÃO */}
          <div style={{ ...card,gridColumn:"2/6",gridRow:"4" }}>
          ...
          <div style={{ ...card,gridColumn:"5",gridRow:"4/6",borderLeft:`3px solid ${t.yellow}`,display:"flex",flexDirection:"column",gap:6 }}>
