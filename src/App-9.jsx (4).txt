...
function Dashboard({ user, displayName, onLogout }) {
  ...
  const addSaving=()=>{ 
    ...
    updGlobal(p=>({...p,savings:[...(p.savings||[]),{id:Date.now(),desc:form.desc,value:val,date:form.date||now.toISOString().slice(0,10)}]}));
    closeModal(); 
  };
  ...
  return (
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div ref={scrollRef} style={{ flex:1,overflowY:"auto",padding:"14px 16px" }}>
        <div style={{ display:"grid",gap:10,gridTemplateColumns:"200px 1fr 1fr 1fr 210px",gridTemplateRows:"repeat(6,minmax(160px,1fr))",alignContent:"start" }}>

          {/* COL 1 */}
          <div style={{ gridColumn:"1",gridRow:"1/8",display:"flex",flexDirection:"column",gap:9 }}>
            ...
            <button onClick={()=>setShowSavings(true)} style={{ background:t.yellow,border:"none",borderRadius:8,color:"#000",padding:"8px 10px",cursor:"pointer",fontSize:11,fontWeight:800,display:"flex",alignItems:"center",gap:6,justifyContent:"center",width:"100%" }}>
              🏦 {tr.savingsBtn} · {fmt(totalSavings)}
            </button>

            {/* METAS */}
            <div style={{ ...card,flex:1,display:"flex",flexDirection:"column",minHeight:120 }}>
                  ...
                  : (global.goals||[]).map(goal=>{
                      ...
                      return (
                        <div key={goal.id} style={{ padding:"7px 8px",borderRadius:8,background:t.bgCard2 }}>
                          ...
                          {!done&&<button onClick={()=>{ const v=prompt("Quanto adicionar?"); if(v&&!isNaN(v)) updGlobal(p=>({...p,goals:(p.goals||[]).map(g=>g.id===goal.id?{...g,current:Math.min(Number(g.current)+Number(v),g.target)}:g)})); }} style={{ ...Gb(t.accent),fontSize:9,padding:"2px 8px",marginTop:5,width:"100%",justifyContent:"center" }}>{tr.addGoalValue}</button>}
                        </div>
                      );
                    })
                }
            ...
            <div style={{ ...card }}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                ...
                <button onClick={refreshRates} style={{ ...Gb(t.accent),padding:"3px 7px",fontSize:10 }}>
                  {loadingRates?"⏳":tr.update}
                </button>
              </div>
              {ratesUpdatedAt&&<div style={{ fontSize:9,color:t.textMuted,marginBottom:6 }}>{tr.updatedAt} {ratesUpdatedAt.toLocaleTimeString(global.language||"pt-BR")}</div>}
              <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                {[
                  ...
                  if(global.currency==="BRL") value=c.code==="USD"?(1/rates.USD):(1/rates.EUR);
                  else if(global.currency==="USD") value=c.code==="BRL"?rates.USD:(rates.EUR/rates.USD);
                  else value=c.code==="BRL"?rates.EUR:(rates.USD/rates.EUR);
                  const formatted=value?value.toLocaleString(global.language||"pt-BR",{minimumFractionDigits:4,maximumFractionDigits:4}):"—";
                  return (
                    <div key={c.code} onClick={()=>updGlobal(p=>({...p,currency:c.code}))}
                      style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 8px",borderRadius:8,background:t.bgCard2,cursor:"pointer" }}
                      onMouseEnter={e=>e.currentTarget.style.background=t.border}
                      onMouseLeave={e=>e.currentTarget.style.background=t.bgCard2}>
                      <span style={{ fontSize:16 }}>{c.flag}</span>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11,color:t.text,fontWeight:700 }}>{c.name}</div>
                        <div style={{ fontSize:12,color:t.accent,fontWeight:800 }}>1 {c.code} = {curSym} {formatted}</div>
                      </div>
          ...
          <div style={{ ...card,gridColumn:"2/3",gridRow:"2",display:"flex",flexDirection:"column" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
            ...
            {filteredExpenses.length===0
              ...
              : <div style={{ display:"flex",flexDirection:"column",maxHeight:"none",overflowY:"auto",flex:1 }}>
          ...
          <div style={{ ...card,gridColumn:"3/4",gridRow:"2",display:"flex",flexDirection:"column" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
          ...
          {/* EVOLUÇÃO */}
          <div style={{ ...card,gridColumn:"2/5",gridRow:"4" }}>
          ...
          {/* INSIGHTS DO MÊS */}
          <div style={{ ...card,gridColumn:"5",gridRow:"5" }}>
            <InsightsCard
      ...
      <Modal open={modal==="goal_new"} onClose={closeModal} title="Nova Meta Financeira" t={t}>
        ...
        <div style={{ display:"flex",gap:10 }}>
          ...
          <input style={{...inp,flex:2}} type="number" placeholder="Valor alvo (R$) *" value={form.target||""} onChange={e=>setForm(p=>({...p,target:e.target.value}))}/>
        </div>
        <input style={inp} type="number" placeholder="Valor já guardado (opcional)" value={form.current||""} onChange={e=>setForm(p=>({...p,current:e.target.value}))}/>
        <button onClick={()=>{
      ...
      <div className="fade-in" onClick={e=>e.stopPropagation()} style={{ background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:16,padding:28,width:360,boxShadow:`0 0 80px ${t.accentGlow}` }}>
        ...
        <div style={{ display:"flex",gap:10 }}>
          ...
          <div style={{ ...card, gridColumn:"2/3", gridRow:"6" }}>
            <div style={{ fontWeight:700, fontSize:13, marginBottom:10 }}>📋 Budget Planner</div>
            <div style={{ fontSize:11, color:t.textSub, marginBottom:8 }}>Limite por categoria</div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {(categories||[]).slice(0,4).map(cat=>{
                const spent = expenses.filter(e=>e.catId===cat.id).reduce((s,e)=>s+Number(e.value||0),0)*rate;
                const limit = (global.budgets||{})[cat.id] || (totalInc*rate*0.2);
                const pct = limit>0 ? Math.min((spent/limit)*100,100) : 0;
                return (
                  <div key={cat.id}>
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, marginBottom:2 }}>
                      <span style={{ color:t.text }}>{cat.icon} {cat.name}</span>
                      <span style={{ color:pct>=100?t.red:t.accent }}>{fmt(spent)}</span>
                    </div>
                    <div style={{ background:t.bgCard2, borderRadius:3, height:4, overflow:"hidden" }}>
                      <div style={{ width:`${pct}%`, height:"100%", background:pct>=100?t.red:t.green, borderRadius:3, transition:"width 0.4s" }}/>
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ ...card, gridColumn:"3/4", gridRow:"6" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                { label:"Entradas", val:totalInc*rate, color:t.green, icon:"⬆" },
                { label:"Saídas", val:totalExp*rate, color:t.red, icon:"⬇" },
                { label:"Investido", val:totalInv*rate, color:t.accent, icon:"📈" },
                { label:"Poupança", val:(global.savings||[]).reduce((s,e)=>s+Number(e.value||0),0)*rate, color:t.yellow, icon:"🏦" },
                { label:"Líquido", val:(totalInc-totalExp-totalInv)*rate, color:(totalInc-totalExp-totalInv)>=0?t.green:t.red, icon:"💰" },
              ].map((item,i)=>(
          ...
          <div style={{ ...card, gridColumn:"4/5", gridRow:"6" }}>
            <div style={{ fontWeight:700, fontSize:13, marginBottom:10 }}>🏆 Top Gastos</div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {[...expenses].sort((a,b)=>Number(b.value)-Number(a.value)).slice(0,5).map((exp,i)=>{
                const cat = categories.find(c=>c.id===exp.catId);
                return (
                  <div key={exp.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"4px 0", borderBottom:`1px solid ${t.border}` }}>
                    <span style={{ fontSize:11, color:t.textMuted, width:14 }}>#{i+1}</span>
                    <span style={{ fontSize:10, flex:1, color:t.text }}>{exp.desc}</span>
                    <span style={{ fontSize:11, color:t.red, fontWeight:700 }}>{fmt(Number(exp.value))}</span>
                  </div>
                );
              })}
              {expenses.length===0 && <div style={{ color:t.textMuted, fontSize:11, textAlign:"center", paddingTop:20 }}>Sem gastos ainda</div>}
            </div>
          </div>

          {/* SCORE FINANCEIRO */}
          <div style={{ ...card, gridColumn:"5", gridRow:"6" }}>
            <div style={{ fontWeight:700, fontSize:13, marginBottom:10 }}>⭐ Score</div>
            {(()=>{
              const savRate = totalInc>0 ? (totalInc-totalExp)/totalInc*100 : 0;
              const invRate = totalInc>0 ? totalInv/totalInc*100 : 0;
              const score = Math.min(100, Math.round(savRate*0.5 + invRate*0.3 + (expenses.length>0?20:0)));
              ...
              return (
                <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:8, paddingTop:8 }}>
                  ...
                  <div style={{ width:"100%", background:t.bgCard2, borderRadius:6, height:8, overflow:"hidden", marginTop:4 }}>
                    <div style={{ width:`${score}%`, height:"100%", background:color, borderRadius:6, transition:"width 0.6s" }}/>
                  </div>
                  <div style={{ fontSize:9, color:t.textMuted, textAlign:"center" }}>
                    Economia: {savRate.toFixed(0)}% · Invest: {invRate.toFixed(0)}%
                  </div>
                </div>
              );
            })()}
