            ...
            <div style={{ ...card }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                ...
                <button onClick={refreshRates} style={{ ...Gb(t.accent), padding:"3px 7px", fontSize:10 }}>
                  {loadingRates ? "..." : `${Ico.refresh} Atualizar`}
                </button>
              </div>
              {ratesUpdatedAt && (
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {[
                  { code:"USD", name:"Dólar", flag:"🇺🇸", val: rates.USD ? (1/rates.USD).toFixed(2) : "—" },
                  { code:"EUR", name:"Euro",  flag:"🇪🇺", val: rates.EUR ? (1/rates.EUR).toFixed(2) : "—" },
                  { code:"BRL", name:"Real",  flag:"🇧🇷", val: rates.USD ? (1/rates.USD).toFixed(2) : "—" },
                ].filter(c => c.code !== global.currency).map(c => {
                  // Show: how many BRL = 1 of this currency (or reverse)
                  let label, value;
                  if(global.currency === "BRL") {
                    // Show how many BRL = 1 USD or 1 EUR
                    const r = c.code==="USD" ? rates.USD : rates.EUR;
                    value = r ? `1 ${c.code} = R$ ${(1/r).toFixed(4)}` : "—";
                  } else if(global.currency === "USD") {
                    if(c.code==="BRL") value = `1 USD = R$ ${rates.USD?(1/rates.USD).toFixed(4):"—"}`;
                    if(c.code==="EUR") value = `1 EUR = US$ ${(rates.EUR&&rates.USD)?(rates.EUR/rates.USD).toFixed(4):"—"}`;
                  } else if(global.currency === "EUR") {
                    if(c.code==="BRL") value = `1 EUR = R$ ${rates.EUR?(1/rates.EUR).toFixed(4):"—"}`;
                    if(c.code==="USD") value = `1 USD = € ${(rates.EUR&&rates.USD)?(rates.USD/rates.EUR).toFixed(4):"—"}`;
                  }
                  return (
                    <div key={c.code} style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 8px", borderRadius:8, background:t.bgCard2, cursor:"pointer" }}
                      onClick={()=>updGlobal(p=>({...p,currency:c.code}))}>
                      <span style={{ fontSize:16 }}>{c.flag}</span>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, color:t.text, fontWeight:600 }}>{c.name}</div>
                        <div style={{ fontSize:10, color:t.accent, fontWeight:700 }}>{value}</div>
                      </div>
                      <div style={{ fontSize:9, color:t.textMuted }}>trocar →</div>
                    </div>
                  );
                })}
              </div>
