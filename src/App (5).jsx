...
const DEFAULT_LAYOUT = {
  barchart:      { area:"bc",  label:"Receitas vs Gastos", icon:"📈", col:"2/4", row:"1" },
  categorias:    { area:"cat", label:"Por Categoria",      icon:"🍩", col:"4",   row:"1" },
  carteira:      { area:"crt", label:"Carteira",           icon:"💼", col:"5",   row:"1/4" },
  gastos:        { area:"ga",  label:"Gastos",             icon:"💸", col:"2",   row:"2" },
  receitas:      { area:"re",  label:"Receitas",           icon:"💰", col:"3",   row:"2" },
  acoes:         { area:"ac",  label:"Ações B3",           icon:"📈", col:"4",   row:"2" },
  calendario:    { area:"ca",  label:"Calendário",         icon:"📅", col:"2",   row:"3" },
  fiis:          { area:"fi",  label:"FIIs",               icon:"🏢", col:"3",   row:"3" },
  investimentos: { area:"inv", label:"Investimentos",      icon:"📊", col:"4",   row:"3" },
  poupanca:      { area:"po",  label:"Poupança",           icon:"🏦", col:"5",   row:"4" },
  evolucao:      { area:"ev",  label:"Evolução Anual",     icon:"📉", col:"2",   row:"4" },
  hoje:          { area:"hj",  label:"Hoje",               icon:"📅", col:"3",   row:"4" },
  insights:      { area:"ins", label:"Insights",           icon:"🧠", col:"4",   row:"4" },
  budget:        { area:"bg",  label:"Budget Planner",     icon:"📋", col:"2",   row:"5" },
  resumo:        { area:"rs",  label:"Resumo do Mês",      icon:"📊", col:"3",   row:"5" },
  topgastos:     { area:"tg",  label:"Top Gastos",         icon:"🏆", col:"4",   row:"5" },
  score:         { area:"sc",  label:"Score",              icon:"⭐", col:"5",   row:"5" },
};

const ROWS = ["1","2","3","4","5"];
const COLS = ["2","3","4","5"];
const COL_LABELS = { "2":"Col A","3":"Col B","4":"Col C","5":"Col D" };
const ROW_LABELS = { "1":"Linha 1","2":"Linha 2","3":"Linha 3","4":"Linha 4","5":"Linha 5" };


// ─── RESIZABLE CARD ──────────────────────────────────────────────────────────
function ResizableCard({ id, editing, sizes, onResize, style, children }) {
  ...
  const cardStyle = {
    ...style,
    position: "relative",
    width:  savedSize.w || style.width  || "100%",
    height: savedSize.h || style.height || "auto",
    minWidth: 80,
    minHeight: 60,
    flexShrink: 0,
    overflow: "hidden",
    boxSizing: "border-box",
    ...(editing ? { outline: "2px dashed #e53935", outlineOffset: 2 } : {}),
  };
  ...
  const startResize = (e) => {
    ...
    const onMove = (e) => {
      ...
      const newW = Math.max(80,  Math.round((startPos.current.w + t.clientX - startPos.current.x) / 20) * 20);
      const newH = Math.max(60, Math.round((startPos.current.h + t.clientY - startPos.current.y) / 20) * 20);
        ...
        <>
          ...
          <div
            ...
            style={{
              ...
              width:20, height:20, cursor:"se-resize", zIndex:999,
          ...
          <div style={{
            ...
            padding:"1px 5px", pointerEvents:"none", zIndex:998,
          }}>
            {id}
          </div>
        </>
...
function Dashboard({ user, displayName, onLogout }) {
  ...
  const [layoutSizes,setLayoutSizes]=useState(()=>global.layoutSizes||{});
  ...
  return (
    <div style={{ minHeight:"100vh",background:t.bg,color:t.text,fontFamily:"'DM Sans','Segoe UI',sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ background:t.header,borderBottom:`1px solid ${t.border}`,padding:"0 18px",height:54,display:"flex",alignItems:"center",gap:10,flexShrink:0,position:"sticky",top:0,zIndex:200 }}>
        ...
        <button onClick={()=>{ 
          if(editingLayout) updGlobal(p=>({...p,layoutSizes})); 
          setEditingLayout(e=>!e); 
        }} style={{ ...Gb(editingLayout?t.green:t.textSub),padding:"5px 10px",fontSize:12,lineHeight:1,fontWeight:700 }}>{editingLayout?"✅ Confirmar Layout":"📐 Editar Layout"}</button>
    ...
    gridTemplateColumns:"180px 1fr 1fr 1fr 200px",
    gridTemplateRows:"auto auto auto auto auto",
    gridTemplateAreas:(()=>{
      const L={...DEFAULT_LAYOUT,...(global.layout||{})};
      const grid={};
      Object.values(L).forEach(c=>{
        const rows=c.row.includes('/')?[...Array(Number(c.row.split('/')[1])-Number(c.row.split('/')[0])).keys()].map(i=>String(Number(c.row.split('/')[0])+i)):[c.row];
        rows.forEach(r=>{ if(!grid[r]) grid[r]={}; grid[r][c.col]=c.area; });
      });
      // sidebar always col1
      ["1","2","3","4","5"].forEach(r=>{ if(!grid[r]) grid[r]={}; grid[r]["1"]="sb"; });
      return ["1","2","3","4","5"].map(r=>
        '"'+["1","2","3","4","5"].map(c=>grid[r]?.[c]||".").join(" ")+'"'
      ).join(" ");
    })()
  }}>

          {/* COL 1 */}
          <div style={{ width:180,flexShrink:0,display:"flex",flexDirection:"column",gap:8,overflowY:"auto",gridArea:"sb"}}>
            ...
            {/* POUPANÇA */}
            <button onClick={()=>setShowSavings(true)} style={{ background:t.yellow,border:"none",borderRadius:8,color:"#000",padding:"8px 10px",cursor:"pointer",fontSize:11,fontWeight:800,display:"flex",alignItems:"center",gap:6,justifyContent:"center",width:"100%",flexShrink:0 ,gridArea:"po"}}>
          ...
          {/* GRÁFICO MENSAL */}
          <ResizableCard id="bc" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:2,minWidth:0,minHeight:90,overflow:"hidden",display:"flex",flexDirection:"column" ,gridArea:"bc" }}>
          ...
          {/* DONUT CATEGORIAS */}
          <div style={{ ...card,...card,width:240,flexShrink:0,minHeight:90,overflow:"hidden",alignSelf:"start",display:"flex",flexDirection:"column",overflow:"hidden",alignSelf:"start",maxHeight:160,height:160 ,gridArea:"cat"}}>
            <ResizableCard id="cat" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ fontWeight:700,fontSize:12,marginBottom:4  }}>{tr.byCategory}</ResizableCard>
            {expByCat.length===0
          ...
          {/* CARTEIRA com total */}
          <div style={{ ...card,...card,width:210,flexShrink:0,display:"flex",flexDirection:"column",overflow:"hidden",alignSelf:"start" ,gridArea:"crt"}}>
            <ResizableCard id="crt" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ fontWeight:700,fontSize:13,marginBottom:4  }}>{tr.wallet}</ResizableCard>
          ...
          <ResizableCard id="ga" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,display:"flex",flexDirection:"column",overflow:"hidden",overflow:"hidden",alignSelf:"start",display:"flex",flexDirection:"column" ,gridArea:"ga" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
          ...
          <ResizableCard id="re" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,display:"flex",flexDirection:"column",overflow:"hidden",overflow:"hidden",alignSelf:"start",display:"flex",flexDirection:"column" ,gridArea:"re" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
          ...
          <ResizableCard id="ca" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,display:"flex",flexDirection:"column",overflow:"hidden" ,gridArea:"ca" }}>
            <FinancialCalendar
          ...
          {/* AÇÕES */}
          <ResizableCard id="ac" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,display:"flex",flexDirection:"column",overflow:"hidden" ,gridArea:"ac" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
          ...
          {/* FIIs */}
          <ResizableCard id="fi" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,display:"flex",flexDirection:"column",overflow:"hidden" ,gridArea:"fi" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
          ...
          {/* EVOLUÇÃO */}
          <ResizableCard id="ev" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,overflow:"hidden" ,gridArea:"ev" }}>
          ...
          {/* INVESTIMENTOS LISTA */}
          <ResizableCard id="inv" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,display:"flex",flexDirection:"column",overflow:"hidden" ,gridArea:"inv" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9 }}>
            ...
            {/* HOJE */}
          <ResizableCard id="hj" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,overflow:"hidden",borderLeft:`3px solid ${t.yellow}`,display:"flex",flexDirection:"column",gap:6 ,gridArea:"hj" }}>
          ...
          {/* POUPANÇA — col5 row4 */}
          <ResizableCard id="po" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ background:t.yellow,...card,width:100,flexShrink:0,overflow:"hidden",alignSelf:"start",borderRadius:12,padding:"13px 15px",display:"flex",flexDirection:"column",gap:8,cursor:"pointer" }} onClick={()=>setShowSavings(true)}>
            <div style={{ fontWeight:800,fontSize:13,color:"#000"  }}>🏦 Poupança</ResizableCard>
          ...
          {/* INSIGHTS */}
          <ResizableCard id="ins" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,width:140,flexShrink:0,overflow:"hidden" ,gridArea:"ins" }}>
            <InsightsCard
          ...
          {/* BUDGET PLANNER */}
          <ResizableCard id="bg" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,width:120,flexShrink:0,overflow:"hidden" ,gridArea:"bg" }}>
          ...
          {/* RESUMO DO MÊS */}
          <ResizableCard id="rs" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,flex:1,minWidth:0,overflow:"hidden" ,gridArea:"rs" }}>
          ...
          {/* TOP GASTOS */}
          <ResizableCard id="tg" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,width:120,flexShrink:0,overflow:"hidden" ,gridArea:"tg" }}>
          ...
          {/* SCORE FINANCEIRO */}
          <ResizableCard id="sc" editing={editingLayout} sizes={layoutSizes} onResize={handleResize} style={{ ...card,...card,width:100,flexShrink:0,overflow:"hidden" ,gridArea:"sc" }}>
          ...
          {/* CÂMBIO CARD */}
          <div style={{ ...card,...card,width:180,flexShrink:0,overflow:"hidden" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6 }}>
              <div style={{ fontWeight:700,fontSize:11 }}>💱 {tr.liveExchange}</div>
              <button onClick={refreshRates} style={{ ...Gb(t.textSub),padding:"2px 6px",fontSize:9 }}>{loadingRates?"⏳":tr.update}</button>
            </div>
            {[{code:"USD",flag:"🇺🇸",name:"Dólar"},{code:"EUR",flag:"🇪🇺",name:"Euro"}].filter(c=>c.code!==global.currency).map(c=>{
              const v=c.code==="USD"?(1/rates.USD):(1/rates.EUR);
              return (
                <div key={c.code} style={{ display:"flex",alignItems:"center",gap:6,padding:"4px 6px",background:t.bgCard2,borderRadius:7,marginBottom:4 }}>
                  <span>{c.flag}</span>
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:9,color:t.textSub }}>{c.name}</div>
                    <div style={{ fontSize:11,color:t.accent,fontWeight:800 }}>1 {c.code} = R$ {v?v.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}):"—"}</div>
                  </div>
                </div>
              );
            })}
...
// ─── LAYOUT EDITOR ─────────────────────────────────────────────────────────────
function LayoutEditor({ layout, onSave, t }) {
  ...
  const getCardAt = (col, row) =>
    Object.entries(cards).find(([,c]) => c.col === col && c.row === row);

  const handleDrop = (col, row) => {
    if (!dragging) return;
    const existing = getCardAt(col, row);
    setCards(prev => {
      const next = { ...prev };
      if (existing) {
        const [existId, existCard] = existing;
        next[existId] = { ...existCard, col: prev[dragging].col, row: prev[dragging].row };
      }
      next[dragging] = { ...prev[dragging], col, row };
      return next;
    });
    setDragging(null);
    setSaved(false);
  ...
  const cellStyle = (col, row, hasCard) => ({
    border: `2px dashed ${hasCard ? "transparent" : "#333"}`,
    borderRadius: 8,
    minHeight: 56,
    background: hasCard ? "transparent" : "#0d0d0d",
    display: "flex",
    alignItems: "center",
  ...
  const cardStyle = (isDragging) => ({
    background: isDragging ? "#e53935" : "#252525",
    border: `1px solid ${isDragging ? "#e53935" : "#333"}`,
    borderRadius: 8,
    padding: "6px 10px",
    cursor: "grab",
    display: "flex",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      <div style={{ fontSize:11, color:"#888", textAlign:"center" }}>
        Arraste os cards para reorganizar o layout. Salva automaticamente no seu perfil.
      </div>
      ...
      <div style={{
        ...
        gridTemplateRows: "24px repeat(5, auto)",
        gap: 4,
      }}>
        {/* Header row */}
        <div/>
        {COLS.map(col => (
          <div key={col} style={{ fontSize:9, color:"#555", textAlign:"center", fontWeight:700, letterSpacing:1 }}>
            {COL_LABELS[col]}
          </div>
        ))}
        ...
        {ROWS.map(row => (
          <>
            <div key={"rl"+row} style={{ fontSize:9, color:"#555", display:"flex", alignItems:"center", fontWeight:700 }}>
              {ROW_LABELS[row]}
            </div>
            {COLS.map(col => {
              const found = getCardAt(col, row);
              const [cardId, cardData] = found || [];
              return (
                <div
                  key={col+row}
                  style={cellStyle(col, row, !!found)}
                  onDragOver={e => { e.preventDefault(); e.currentTarget.style.background="#1a1a1a"; }}
                  onDragLeave={e => { e.currentTarget.style.background = found ? "transparent" : "#0d0d0d"; }}
                  onDrop={e => { e.currentTarget.style.background = found ? "transparent" : "#0d0d0d"; handleDrop(col, row); }}
                >
                  {found && (
                    <div
                      style={cardStyle(dragging === cardId)}
                      draggable
                      onDragStart={() => setDragging(cardId)}
                      onDragEnd={() => setDragging(null)}
                    >
                      <span style={{ fontSize:14 }}>{cardData.icon}</span>
                      <span style={{ fontSize:9, lineHeight:1.2 }}>{cardData.label}</span>
                    </div>
                  )}
                </div>
              );
      ...
      {/* Unplaced cards */}
      {(() => {
        const placed = new Set(Object.values(cards).map(c => c.col+c.row));
        const all = new Set(COLS.flatMap(c => ROWS.map(r => c+r)));
        // Check if any card is out of valid range
        const misplaced = Object.entries(cards).filter(([,c]) =>
          !COLS.includes(c.col) || !ROWS.includes(c.row)
        );
        if (misplaced.length === 0) return null;
        return (
          <div style={{ background:"#111", borderRadius:8, padding:8 }}>
            <div style={{ fontSize:10, color:"#555", marginBottom:6 }}>Cards fora do grid:</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
              {misplaced.map(([id, c]) => (
                <div key={id} style={cardStyle(dragging===id)} draggable
      ...
      <div style={{ display:"flex", gap:8, marginTop:4 }}>
        <button onClick={handleReset} style={{
          ...
          borderRadius:8, padding:"8px", fontSize:12, color:"#888", cursor:"pointer"
        }}>↺ Resetar padrão</button>
        <button onClick={handleSave} style={{
          flex:2, background: saved ? "#2e7d32" : "#e53935",
          border:"none", borderRadius:8, padding:"8px",
          fontSize:12, fontWeight:700, color:"#fff", cursor:"pointer",
          transition:"background 0.3s"
        }}>
          {saved ? "✅ Layout salvo!" : "💾 Salvar Layout"}
        </button>
      </div>
    </div>
  );
    ...
    <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)" }}>
      <div className="fade-in" onClick={e=>e.stopPropagation()} style={{ background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:18,padding:24,width:420,maxHeight:"85vh",display:"flex",flexDirection:"column",boxShadow:`0 0 80px ${t.accentGlow}` }}>
        ...
        <div style={{ display:"flex",gap:6,marginBottom:16 }}>
          ...
          <button style={tabStyle(tab==="layout")} onClick={()=>setTab("layout")}>📐 Layout</button>
        </div>
        ...
        {tab==="layout" && (
          <LayoutEditor
            layout={g.layout||{}}
            onSave={(newLayout)=>{ updGlobal(p=>({...p,layout:newLayout})); }}
            t={t}
          />
        )}
      </div>
